...
> [key] **The loop** — **C-E-B-O-C-V:** **C**larify → **E**xamples → **B**rute force → **O**ptimize → **C**ode → **V**erify. Spend the first third of the interview *before* writing real code. Silence and a blank screen read as "stuck"; a narrated plan reads as "senior."
...
| 3 | **Brute force** | 1–2 min | State the naive solution and its complexity — *don't* code it. Name the **target** complexity from the input size (see the [complexity heuristic](#complexity-amortization-the-cost-model)). | *"Brute force is O(n²) pair-checking. n is 10⁵, so O(n²) ≈ 10¹⁰ is too slow — I need O(n) or O(n log n)."* |
| 4 | **Optimize** | 2–4 min | Route to a pattern via the [decision tree](#the-which-pattern-decision-tree). State the **key insight and invariant before coding**. Get buy-in. | *"Because it's sorted, two converging pointers discard a whole side each step — O(n). The invariant is: the answer, if any, lies between `lo` and `hi`."* |
...
> [pat] **What separates a hire from a strong hire** — at L5+/staff, the bar isn't "solved it." It's these:
...
> [trap] **Avoid these — they read as "not senior"** — silent coding with no narration; jumping to code before stating an approach; claiming a complexity you can't defend; never considering the empty/single/overflow case; and — the most common — *not noticing your own bug* on a trivial input the interviewer then has to point out.
...
> [key] **How to choose** — all three tracks follow the same order — **Foundations → Patterns → Depth** — they differ only in how many canonical problems per pattern you drill. Depth beats breadth: 3 problems you can reproduce cold beat 12 you half-remember.

| | **2-Week Sprint** (interview imminent) | **4-Week Standard** | **8-Week Mastery** |
|---|---|---|---|
| **Days 1–2** | Skim Part I primer + Complexity; memorize the [decision tree](#the-which-pattern-decision-tree). | Part I in full; do the *Easy* structure warm-ups. | Part I in full + re-implement each structure from scratch once. |
...
| **Last 2 days** | Part II + Part V cheat sheets; the [self-check drills](#appendix-self-check-mastery-drills). | Mock interviews using the [6-phase loop](#the-interview-playbook). | Timed mocks + re-drill every problem you missed. |
