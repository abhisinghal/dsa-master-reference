...
> [note] **Trace it** — pattern `"ABAB"` has `lps = [0,0,1,2]`: after matching `"ABAB"` then hitting a mismatch, you fall back to `lps[3]=2` (the prefix `"AB"` already re-matched), so the text pointer never rewinds.
