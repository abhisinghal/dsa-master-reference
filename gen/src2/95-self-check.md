...
> [note] **How to score yourself.** Instant + correct = mastered. Correct but slow (&gt;10 s) = fragile, re-drill weekly. Wrong = go back to that chapter's intro and the "Recognition signals" line. Aim for **instant** on the whole Decision Tree before your interview.
...
> [trap] If you answered "sort it" for #19, notice the O(n) hash approach beats O(n log n) — the word *consecutive* plus *unsorted* is the tell for a **set**, not a sort.
...
> [key] The two classic traps: a **geometric** sum (#2, #6) collapses to O(largest term), while a **log-many-equal-terms** sum (#1) gives the extra log factor. Ask "do the terms *shrink* or *stay equal*?" — that one question separates O(n) from O(n log n).
...
> [key] **The only mantra that matters:** name the *family* first (contiguous? sorted? dependencies? overlapping subproblems?), and the specific algorithm falls out of it. Interviewers reward the engineer who says "this is a shortest-path problem" before writing a line — because that sentence is the actual skill.
