...
**The one thing that changes vs the flagship for this pattern:** treat `nums[i]` as a next pointer and return the cycle entry value
...
1. **Read the pattern chapter's `Find the Duplicate Number` section** — brute force, Java code, and Execution Trace are already there.
