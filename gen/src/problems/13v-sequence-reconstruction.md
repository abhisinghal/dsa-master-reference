...
**The one thing that changes vs the flagship for this pattern:** Require the queue to have exactly one choice at every step to prove the order is unique.
...
1. **Read the pattern chapter's `Sequence Reconstruction` section** — brute force, Java code, and Execution Trace are already there.
