...
> [key] **Recommended path:** primer (mechanics) → patterns (practice) → topic chapters (depth). **Don't** read the topic problem-chapters cold before the patterns — that's exactly the confusion to avoid.

> [pat] **Start here:** before your first problem, read **[The Interview Playbook](#the-interview-playbook)** — the 6-phase loop (Clarify → Examples → Brute force → Optimize → Code → Verify) that scores you as *senior* — and pick a track in **[Study Plans &amp; Revision Cadence](#study-plans-revision-cadence)**. Track what you've mastered in the **[Master Problem Index](#master-problem-index-tracker)**.

> [note] **Prerequisite chain** — a few patterns stack on earlier ones. Learn them in this dependency order: **recursion → backtracking → dynamic programming**; **arrays & hashing → prefix sum → sliding window**; **heaps → Dijkstra & K-way merge**; **stacks → monotonic stack**; **trees → tree-DP & tries**. If a pattern feels impossible, you're usually missing its prerequisite, not the pattern itself.
...
> [key] **Key Insight** (blue) — the one idea that unlocks the problem.

> [inv] **Invariant** (amber) — the property held true at every step; the backbone of correctness.

> [trap] **Common Trap** (red) — the mistake that fails the interview.

> [pat] **Pattern Connection** (green) — how this generalizes to the larger family.
