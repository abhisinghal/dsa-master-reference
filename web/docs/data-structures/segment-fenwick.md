...
Each Fenwick index `i` stores the sum of `a[i - (i&-i) + 1 .. i]`. To update, walk *up* adding `i += i & -i`; to query a prefix, walk *down* subtracting `i -= i & -i`. Range sum `[l,r] = query(r) − query(l-1)`.
...
`prefixSum(6)` visits indices `6 → 4 → 0` (each step strips the lowest set bit via `i -= i & -i`), summing three stored partials instead of six elements.
...
0-index vs 1-index confusion. *Example:* `update(i)` for `i=0` with 0-indexed `i` gives `i & -i == 0`, so the loop never advances. Fenwick trees are naturally 1-indexed; shift external indices by +1 or handle the 0 case explicitly.
...
Each internal node's value is the aggregate (sum/min/max) of its two children; a pending lazy tag means "this node's value is current, but children haven't been informed yet."
...
over `[1,3,5,7,9,11]`, a query for `sum(1..3)` combines two canonical nodes (covering `[1,1]` and `[2,3]`) → `3 + (5+7) = 15`, touching O(log n) nodes instead of scanning the range.
...
Segment trees back *Range Sum/Min/Max Query — Mutable*, *Range Add + Range Sum* (lazy), *The Skyline Problem* (segment tree on heights), and *Count of Range Sum*. When only prefix sums with point updates are needed, prefer the simpler, faster Fenwick tree.
...
The iterative bottom-up segment tree above is compact and cache-friendly for point updates. For **range updates** (add v to `[l,r]`), add lazy tags and a recursive `push-down`: apply the tag to a node, mark its children pending, and only propagate when recursing into them.
