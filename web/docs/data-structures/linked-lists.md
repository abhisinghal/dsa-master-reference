...
<Callout kind="note" title="The interview reality">

Most linked-list questions do not ask you to implement a collection from scratch. They give you the first node, `head`, and ask you to return a node after traversing or rewiring the chain.
...
<Callout kind="trap" title="Reference assignment does not copy a list">

`ListNode cur = head` creates a second reference to the same first node. Mutating `cur.next` changes the original chain. Advancing `cur = cur.next` changes only the local reference, not `head`.
