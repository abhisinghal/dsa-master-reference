...
A trie separates two questions: "can I walk this prefix?" and "is this prefix a complete stored word?" The path answers the first; the terminal flag answers the second.
...
<Callout kind="inv" title="Implementation invariant">

every edge corresponds to exactly one character, and the node reached after consuming a string represents that exact prefix. `isWord` is independent of whether the node has children.
...
In interviews, use the 26-array when constraints say lowercase English. Mention the map version if the interviewer changes the alphabet.
...
`isEnd` only on leaves. *Example:* insert `"car"` then `"cars"`. If you only mark `s` as end, `search("car")` returns false. `isEnd` marks a **word boundary**, independent of children — set it on `r` too.
...
<Callout kind="trap" title="Deletion Trap">

Never prune a node just because the deleted word used it. Prune only when no word ends there and no child remains, or when a maintained `pass` count drops to zero.
...
<Callout kind="key" title="Final mental model">

A trie is not magic compression and it is not always smaller than a set. It buys predictable prefix-time by paying for nodes along shared paths. Use it when prefix behavior is the product requirement, not just because strings are present.
