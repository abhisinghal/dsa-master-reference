...
You're an intern at a scheduling startup. The founder walks up: *"Given a list of meeting requests with start and end times, what's the maximum number of non-overlapping meetings I can pack into one conference room?"*

The obvious approach: try every subset of meetings. Check each for validity (no overlaps). Return the largest valid subset. There are `2ⁿ` subsets — for `n = 30` meetings, that's `10⁹` combinations. Your laptop chokes for a minute, then returns the right answer. It's correct. It's provably optimal. And for `n ≤ 20` it's actually a legit answer that many textbooks use as their reference for scheduling problems.

But at `n = 100` meetings (a large enterprise's daily calendar), brute force is `10³⁰` subsets — **more than the age of the universe** in nanoseconds. And the killer insight is: you don't need to try every subset. You just need to answer *"which meeting should I pick next?"* at each step. If you always pick the meeting that **ends earliest** among all still-available meetings, you provably get the maximum count. **Sort by end time, sweep left to right, greedily pick any meeting that doesn't overlap the last picked.** O(n log n). For 100 meetings, that's ~700 ops — a microsecond. From universe-age to microsecond, by making the *right* local choice.

That's greedy: **grab the best-looking option right now and never look back**. When it works, the code is embarrassingly short (usually just *sort, then sweep once*). The catch is that "best right now" isn't always "best overall," so a greedy algorithm is only correct when two properties hold: the **greedy-choice property** (a locally optimal pick is safe to commit to) and **optimal substructure** (what's left is a smaller version of the same problem). In an interview the code is the easy part; the points come from *proving* it — most often with an **exchange argument**, where you show that any optimal solution can be nudged, one swap at a time, into the greedy one without ever getting worse.
...
Before coding greedy, find the sort key that makes the safe choice obvious (earliest finish, largest ratio, nearest deadline). If you can construct a counterexample to "always take the locally best," greedy is wrong → switch to DP.
...
### When NOT to use it
A locally-best choice can be regretted later — construct a counterexample ("if I take the largest coin first…") and if you find one, switch to [Dynamic Programming](/patterns/dp). The exchange-argument proof is what separates a safe greedy from a wrong one.
...
`curEnd` is the boundary of positions reachable in `jumps` steps; `farthest` is the boundary reachable in `jumps+1`. When `i` hits `curEnd`, one more jump is forced.
...
`[2,3,1,1,4]`. From index 0 (reach 2) the best next hop is index 1 (which reaches index 4). Two jumps `0→1→4` → answer **2**.
...
"I first confirm the end is reachable and I need the minimum number of jumps. I start with brute force DFS or BFS over all jumps, which can be exponential for DFS or O(n²) for naive BFS. I optimize by tracking the current level boundary and farthest next reach in one pass, giving O(n) time and O(1) space."
...
Each "level" is the set of indices reachable with the same number of jumps; taking the farthest reach is provably optimal because it dominates every other choice's future reach.

</Callout>

<Callout kind="trap" title="Common Trap">

Counting jumps at every step instead of at the frontier. *Example:* `nums=[2,3,1,1,4]`. Incrementing `jumps` at each index gives 5; incrementing only when `i == currentEnd` (frontier boundary) gives 2. Update `currentEnd = farthest` and `jumps++` together.
...
If you run out of gas going from `start` to `i`, no station in `[start, i]` can be the answer (each had ≤ 0 surplus to offer) → restart at `i+1`. Total feasibility is a separate global check.

</Callout>

<Callout kind="inv" title="Invariant">

`tank` is the surplus since the current candidate start; the moment it goes negative, every earlier candidate is eliminated.
...
`gas=[1,2,3,4,5], cost=[3,4,5,1,2]`. Running balance bottoms out entering index 3, so start at **3**: `4→5→1→2→3` never dips below zero.
...
"Reset the start when the running sum dips" mirrors Kadane's max-subarray reset — both discard a prefix that can only hurt.
...
Greedy interval problems almost always sort by *finish* time (maximize count) or *start* time (merge/coverage). Choosing the wrong key is the classic greedy failure.

</Callout>

<Callout kind="pat" title="Pattern Connection">

Earliest-finish selection underlies *Non-overlapping Intervals* (min removals) and *Minimum Arrows to Burst Balloons* (max non-overlap = min points).

</Callout>

<Callout kind="note" title="Trace it">

tasks `AAABBB, n=2`. The three `A`s frame the timeline `A??A??A`; `B`s and idles fill the gaps → `AB_AB_AB` = **8** intervals.
...
<Callout kind="inv" title="Invariant test">

Greedy is safe iff you can prove the exchange argument. No proof, construct a small counterexample; if one exists, the problem is DP.
...
Keeping the earliest-ending compatible interval leaves the most room for the rest (exchange argument).
...
`[[1,2],[2,3],[3,4],[1,3]]`. Keeping earliest-finishers `[1,2],[2,3],[3,4]` forces dropping `[1,3]` → **1** removal.
...
Max non-overlapping set = *Minimum Arrows to Burst Balloons* (min points to stab all). Sort-by-end greedy is the shared core.
