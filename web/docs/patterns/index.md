...
<Callout kind="note" title="Worked routing — Example 1.">
...
<Callout kind="note" title="Worked routing — Example 2.">

Problem: *"Minimum eating speed so Koko finishes all bananas in H hours."*<br/>**Q1 · What's the phrasing?** "**minimum X such that a condition holds**" → use Route 2 (start from the goal).<br/>**Q2 · Does the order of the data matter?** No — it's the *answer* (the speed) that is **monotone**: any speed ≥ the answer still finishes in time, so feasibility has a single true/false boundary.<br/>**Verdict:** **Binary Search on the Answer.**
...
|---|---|---|
| "contiguous", "window", "substring" + a limit | [Sliding Window](/patterns/sliding-window) | Longest Substring w/o Repeat |
| "subarray sums to k", "count subarrays" (esp. with negatives) | [Prefix Sum + HashMap](/patterns/prefix-sum) | Subarray Sum Equals K |
| array is **sorted / rotated**, or you need a boundary | [Binary Search](/patterns/binary-search) | Search in Rotated Sorted Array |
| **"minimum/maximum X such that feasible"** (huge range) | [Binary Search on the Answer](/patterns/binary-search) | Koko Eating Bananas |
| a **pair/triplet** with a target in a sortable array | [Two Pointers](/patterns/two-pointers) | 3Sum |
| "next/previous greater or smaller", spans, histogram | [Monotonic Stack](/patterns/monotonic-stack) | Daily Temperatures |
| cycle / middle / nth-from-end in a **linked list** | [Fast / Slow Pointers](/data-structures/linked-lists) | Linked List Cycle II |
| array holds **1..n**, find missing/duplicate in O(1) space | [Cyclic Sort](#cyclic-sort) | Find the Missing Number |
| **"kth"**, "top k", "k closest", "k most frequent" | [Heap (size k) / Quickselect](/data-structures/heaps) | Kth Largest Element |
| **median of a stream** | [Two Heaps](/data-structures/heaps) | Find Median from Data Stream |
| merge **k** sorted lists / smallest covering range | [K-way Merge](/data-structures/heaps) | Merge k Sorted Lists |
| **"intervals"**, "meetings", "overlap", "rooms" | [Sort + Merge / Sweep](#intervals-sweep-line) | Merge Intervals |
| **"prerequisites"**, "build order", cycle in a digraph | [Topological Sort](/data-structures/graphs) | Course Schedule II |
| "islands", "provinces", "connected", "accounts" | [Union-Find / DFS/BFS](/data-structures/graphs) | Number of Provinces |
| shortest path, **unweighted** | [BFS](/data-structures/graphs) | Word Ladder |
| shortest path, **weighted ≥ 0** | [Dijkstra](/data-structures/graphs) | Network Delay Time |
| "generate **all** …", place N queens, partitions | [Backtracking](/patterns/backtracking) | Subsets, N-Queens |
| "**how many ways**", "min/max cost to reach" | [Dynamic Programming](/patterns/dp) | Coin Change |
| "prefix", "dictionary", "autocomplete" | [Trie](/data-structures/trie) | Implement Trie |
| set over **n ≤ 20**, "visit all", "assign each" | [Bitmask DP](/patterns/dp) | Travelling Salesman |
| "single number", parity, XOR of pairs | [Bit Manipulation](/patterns/bit-manip) | Single Number |
...
| Fast / Slow Pointers | [Linked list](#linked-list-nodes-and-javas-linkedlist) | [Linked Lists](/data-structures/linked-lists) |
...
| Top-K / Heap · K-way Merge | [PriorityQueue](#priorityqueuet-the-binary-heap) | [Heaps](/data-structures/heaps) |
| Merge Intervals / Sweep | [Array](#array-int-t) sort · [TreeMap](#treemapkv-treesete-sorted-keys) | — |
| Topological Sort · Union-Find | [ArrayDeque](#arraydequet-stack-and-queue-in-one) · [Array](#array-int-t) | [Graphs](/data-structures/graphs) |
...
| Trie | [Trie](#trie-prefix-tree-a-preview) | [Tries](/data-structures/trie) |
...
|---|---|---|---|---|
| 1 | Sliding Window | Longest/shortest contiguous run under a limit — *"longest substring with ≤ K distinct chars."* | The window's validity is monotone, so you expand and only shrink when broken — visiting each index once instead of re-checking every substring. | [→ Sliding Window](/patterns/sliding-window) |
| 2 | Two Pointers | Find a pair/triplet or partition in a sorted array — *"3 numbers summing to 0."* | Sortedness means a too-big sum can only shrink by moving the right pointer left — each move eliminates a whole set of pairs. | [→ Two Pointers](/patterns/two-pointers) |
| 3 | Fast/Slow Pointers | Cycle, middle, or nth-from-end of a linked list — *"does this list loop?"* | Two speeds must meet inside any loop; with no extra memory, the meeting point plus algebra locates the cycle's start. | [→ Linked Lists · Fast/Slow](/data-structures/linked-lists) |
| 4 | Prefix Sum / Diff Array | Many range sums or "count subarrays with sum k" — *even with negatives.* | A range sum becomes a subtraction of two prefixes; storing prefixes in a map turns "= k" into an O(1) lookup a window can't do. | [→ Prefix Sum](/patterns/prefix-sum) |
...
| 6 | Monotonic Stack | Nearest greater/smaller, spans, histograms — *"days until a warmer day."* | Keeping the stack sorted means each element you pop has just found its answer; every index is pushed/popped once → O(n). | [→ Monotonic Stack](/patterns/monotonic-stack) |
| 7 | Binary Search | Locate a value/boundary in sorted/monotone data — *"first position of x."* | A single true/false test on the midpoint discards half the space each step. | [→ Binary Search](/patterns/binary-search) |
| 8 | Binary Search on Answer | "Least capacity/speed/time that works" — *"slowest speed Koko can eat at."* | Feasibility flips false→true exactly once, so you binary-search the threshold and test each guess in O(n). | [→ Binary Search & Search-on-Answer](/patterns/binary-search) |
| 9 | Top-K / Heap | k largest/smallest/most-frequent — *"3 most frequent words."* | A size-k heap of the opposite polarity keeps exactly the best k, evicting the worst in O(log k). | [→ Heaps · Top-K](/data-structures/heaps) |
| 10 | K-way Merge | Merge k sorted lists / smallest covering range — *"merge k sorted linked lists."* | A heap of each list's current head always surfaces the global next-smallest. | [→ Heaps · K-way Merge](/data-structures/heaps) |
...
| 13 | Topological Sort | Order tasks with prerequisites — *"course schedule with deps."* | Repeatedly emit a node whose prerequisites are all done; if you can't finish, a cycle exists. | [→ Graphs · Topological Sort](/data-structures/graphs) |
| 14 | Union-Find (DSU) | Dynamic connectivity/grouping — *"how many friend circles?"* | Path-compressed parent pointers answer "same group?" in ~O(1) as edges arrive. | [→ Graphs · Union-Find](/data-structures/graphs) |
| 15 | Greedy | "Fewest/most you can" with a safe local choice — *"minimum jumps to the end."* | When an exchange argument proves the locally-best choice is never regretted, commit and never backtrack. | [→ Greedy](/patterns/greedy) |
| 16 | Backtracking | Generate all valid arrangements — *"all subsets / N-Queens."* | DFS the choice tree: choose → recurse → undo, pruning dead branches so the exponent stays tame. | [→ Recursion & Backtracking](/patterns/backtracking) |
| 17 | Divide & Conquer | Answer composes from independent halves — *"count smaller elements to the right."* | Solve halves, then a clever O(n) combine (e.g. counting during a merge) yields the whole. | [→ Divide & Conquer](/patterns/divide-conquer) |
| 18 | Dynamic Programming | "How many ways / min-max cost" with reuse — *"fewest coins to make N."* | Overlapping subproblems + optimal substructure: define a state, solve each once, reuse. | [→ Dynamic Programming](/patterns/dp) |
| 19 | Trie (pattern) | Prefix/dictionary queries — *"autocomplete / does any word start with 'ca'?"* | A path spells a prefix; shared prefixes give O(L) queries independent of dictionary size. | [→ Tries](/data-structures/trie) |
| 20 | Bit Manipulation / Bitmask | Parity/sets over ≤ 20 items — *"the one number that isn't duplicated."* | Bits are a set/vector: XOR cancels pairs; a mask encodes "which elements are used." | [→ Bit Manipulation](/patterns/bit-manip) |
| 21 | Quickselect | kth smallest/largest, one-shot — *"kth largest element."* | Partition puts the pivot at its final rank; recurse only into the side containing k → O(n) average. | [→ Quickselect](/patterns/quickselect) |

<Callout kind="key" title="How to use this book">
