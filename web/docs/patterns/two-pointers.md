...
<svg preserveAspectRatio="xMidYMid meet" role="img" viewBox="0 0 400 250" xmlns="http://www.w3.org/2000/svg" font-family="var(--dsa-font)" aria-label="Diagram illustrating: 3Sum Medium">
  <defs>
...
For fixed `i`, `[lo,hi]` brackets all unexplored pairs summing toward `-a[i]`; sortedness makes each move monotone.

</Callout>

<Callout kind="note" title="Trace it">

`[-1,0,1,2,-1,-4]` sorts to `[-4,-1,-1,0,1,2]`. Fix `-1`, then two pointers on the rest find `0+1` and `-1+2` → triplets `[-1,0,1]` and `[-1,-1,2]`.
...
Missing any of the three duplicate-skips yields repeated triplets. *Example:* `nums=[-1,-1,-1,2]`. Without skipping duplicate pivots you emit `[-1,-1,2]` twice (once per `-1` as pivot); without skipping `lo`/`hi` after a hit, `[0,0,0,0]` emits `[0,0,0]` multiple times.
...
Generalizes to k-Sum by recursion (fix outer indices, two-pointer the base case). 4Sum = one more loop.
...
<div class="svg-figure">
<svg preserveAspectRatio="xMidYMid meet" role="img" viewBox="0 0 400 250" xmlns="http://www.w3.org/2000/svg" font-family="var(--dsa-font)" aria-label="Diagram illustrating: Container With Most Water Medium">
  <defs>
...
Area = `min(h[l],h[r]) × (r−l)`. Width shrinks each step, so only raising the limiting height can help; moving the taller wall can never improve.

</Callout>

<Callout kind="note" title="Trace it">

`[1,8,6,2,5,4,8,3,7]`. Widest pair `8@idx1` and `7@idx8` give `min(8,7)×7 = 49` — the max. Moving the taller side could only shrink width without raising the limiting height.
...
Moving the taller wall can never help. *Example:* `heights=[1,8,6,2,5,4,8,3,7]`, `lo=0(h=1), hi=8(h=7)`. Moving `hi` inward shrinks width and can't raise the min (already `1`). Move the shorter wall — the only move that can improve area.
...
Greedy converging pointers. Contrast with Trapping Rain Water, which *accumulates* water rather than maximizing a single span.
...
After squaring, the array is no longer sorted (e.g. `[-4,-1,0,3]` → `[16,1,0,9]`). But the largest square must come from the most-negative or most-positive value — i.e. an **end**. So compare the two ends, take the larger square, and fill the result **back to front**.

</Callout>

<Callout kind="inv" title="Invariant">

`[left, right]` still spans every element whose square hasn't been placed; `pos` marks the next slot to fill from the right, so the output is built in descending square order.

</Callout>

<Callout kind="note" title="Trace it">

`[-4,-1,0,3,10]`. Compare ends: `(-4)²=16` vs `10²=100` → take 100, then 16, 9, 1, 0; fill the output back-to-front → `[0,1,9,16,100]`.
...
Squaring in place, then sorting. *Example:* `nums=[-4,-1,0,3,10]` → squared `[16,1,0,9,100]` still needs a sort (O(n log n)). Two pointers from the ends fill an output array from the back in O(n).
...
"Merge from both ends because the extreme is at an end" is the same converging-pointer idea as Container With Most Water, and mirrors the merge step of merge sort operating on a fold-point.
...
`[0,low)`=0s, `[low,mid)`=1s, `(high,n)`=2s; `[mid,high]` unprocessed. The three regions only grow.
...
"I first confirm the only values are 0, 1, and 2 and the array must be sorted in place. I start with brute force as counting buckets or library sort, which is O(n) in two passes or O(n log n) with sort. I optimize with `low`, `mid`, and `high` regions so one scan partitions the array in O(n) time and O(1) space."
...
Advancing `mid` after swapping with `high` skips an unexamined value. *Example:* `[2,0,2]`, `mid=1`. Swap `a[mid]` with `a[high]` → `[2,0,2]` (a `2` moves in at `mid`). If you `mid++`, you miss re-evaluating that new value. Advance `mid` only when you swapped with `low` or saw a `1`.
...
Partitioning is the core of Quickselect/quicksort; the same 3-way scheme handles duplicate-heavy pivots (3-way quicksort).
...
Water above `i` = `min(maxLeft, maxRight) − h[i]`. Advance from the side with the smaller max, because that side's bound is fixed regardless of what lies beyond.

</Callout>

<Callout kind="inv" title="Invariant">

If `leftMax ≤ rightMax`, the water at `left` depends only on `leftMax` (a taller-or-equal wall is guaranteed on the right).

</Callout>

<Callout kind="note" title="Trace it">

`[0,1,0,2,1,0,1,3,2,1,2,1]` traps **6** units. Over the `0` at index 2, water rises to `min(leftMax=1, rightMax=3)=1`, so 1 unit sits there; summing every bar gives 6.
...
Local vs global boundaries. *Example:* `heights=[4,2,0,3,2,5]`. Water above index 3 (h=3) is bounded by the global `4` on the left and `5` on the right — not by 3. Track running max from each side (or the shorter side with two pointers).
...
Also solvable with a monotonic (decreasing) stack that resolves trapped basins between bars — a good cross-check of the two viewpoints.
