# Bit Manipulation — Sum of All Subset XOR / SOS DP

*[↗ LeetCode: Sum of All Subset XOR / SOS DP](https://leetcode.com/problems/sum-of-all-subset-xor-totals/)* · <span class="diff diff-m">Medium</span> · [pattern chapter →](/patterns/bit-manip)

**The one thing that changes vs the flagship for this pattern:** enumerate submasks with `for (s = m; s > 0; s = (s-1) & m)`
...
1. **Read the pattern chapter's `Sum of All Subset XOR / SOS DP` section** — brute force, Java code, and Execution Trace are already there.
