...
**The one thing that changes vs the flagship for this pattern:** streaming version — push `(price, span)` and collapse spans of smaller prices as they arrive
...
1. **Read the pattern chapter's `Online Stock Span` section** — brute force, Java code, and Execution Trace are already there.
