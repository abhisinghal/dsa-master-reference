...
*[↗ LeetCode: Reverse Pairs](https://leetcode.com/problems/reverse-pairs/)* · <span class="diff diff-m">Medium</span> · [pattern chapter →](/patterns/divide-conquer)

**The one thing that changes vs the flagship for this pattern:** Before merging, count pairs with `a[i] > 2·a[j]`, using `long` to avoid overflow.
...
1. **Read the pattern chapter's `Reverse Pairs` section** — brute force, Java code, and Execution Trace are already there.
