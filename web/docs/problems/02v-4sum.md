...
**The one thing that changes vs the flagship for this pattern:** add one more outer loop, then two-pointer the inner pair (skip duplicates at every level)
...
1. **Read the pattern chapter's `4Sum` section** — brute force, Java code, and Execution Trace are already there.
