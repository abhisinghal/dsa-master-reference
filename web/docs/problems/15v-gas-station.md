...
**The one thing that changes vs the flagship for this pattern:** reset the start to `i+1` when the tank dips below 0, with a global feasibility gate
...
1. **Read the pattern chapter's `Gas Station` section** — brute force, Java code, and Execution Trace are already there.
