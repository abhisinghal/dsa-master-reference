...
**The one thing that changes vs the flagship for this pattern:** two sorted lists; each overlap is `[max(starts), min(ends)]`, then advance the earlier end
...
1. **Read the pattern chapter's `Interval List Intersections` section** — brute force, Java code, and Execution Trace are already there.
