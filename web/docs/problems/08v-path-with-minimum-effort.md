...
**The one thing that changes vs the flagship for this pattern:** binary-search the max allowed step height; feasibility is a BFS/DFS connectivity check
...
1. **Read the pattern chapter's `Path With Minimum Effort` section** — brute force, Java code, and Execution Trace are already there.
