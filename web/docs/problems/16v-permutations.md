...
**The one thing that changes vs the flagship for this pattern:** order matters, so drop the `start` index and instead track a `used[]` array, scanning all positions each level
...
1. **Read the pattern chapter's `Permutations` section** — brute force, Java code, and Execution Trace are already there.
